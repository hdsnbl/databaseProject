...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:100901huds@localhost/databaseProject'
db = SQLAlchemy(app)
CORS(app)
...
class Game(db.Model):
    ...
    users = db.relationship('User', secondary=favorites_association, back_populates='favorite_games')
...
class User(db.Model):
    ...
    # Many-to-many relationship with favorites
    favorite_games = db.relationship('Game', secondary=favorites_association, back_populates='users')
